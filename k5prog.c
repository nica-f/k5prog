...
int k5_readmem(int fd, unsigned char *buf, unsigned char maxlen, int offset)
{
	...
	if (verbose > 1)
		printf("@@@@@@@@@@@@@@@@@@     readmem offset=0x%4.4x len=0x%2.2x\n", offset, maxlen);
...
int k5_writemem(int fd, unsigned char *buf, unsigned char len, int offset)
{
	...
	if (verbose>1)
		printf("@@@@@@@@@@@@@@@@@@     writemem offset=0x%4.4x len=0x%2.2x\n",offset,len);
	...
	memcpy((unsigned char *)&writemem+12, buf, len);
...
int k5_writeflash(int fd, unsigned char *buf, int  len, int offset, int max_flash_addr)
{
	...
	//memcpy((unsigned char *)&writeflash+16, buf, len);
...
int k5_prepare(int fd)
{
	...
	/* this is a bit o a problem with people trying to read the radio config in firmware flash mode,
